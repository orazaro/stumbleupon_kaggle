...
def cv_run(rd, X, y):
    ...
    #scores = cross_validation.cross_val_score(rd, X, y, cv=n_cv, ### ORIGINAL
        scoring='roc_auc', 
...
def main():
    (X_all,y,feature_names) = Prepare_2(model=6).fit()
    lentrain = len(y)
    ...
    clf9 = svm.LinearSVC(C=0.01, loss='l2', penalty='l1', dual=False)
