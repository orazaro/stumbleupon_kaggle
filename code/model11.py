...
# -*- coding: utf-8 -*-
"""
    Model 11: LogisticRegression without using SVD (600k features)
"""
...
class Transformer(BaseEstimator, TransformerMixin):
  ...
  def transform(self, Xmask):
    X_df = self.X_all[Xmask,:]
    print "transform X_df:",X_df.shape,
...
class Model11(BaseEstimator):
  def __init__(self):
    pass
  def fit(self, Xmask, y):
    pr = prepare.Prepare_0(model=11, preproc=1, min_df=1, use_svd=False, tfidf=2,
        stemmer=0)
    ...
    clf = lm.LogisticRegression(penalty='l2', dual=True, tol=0.00001, 
